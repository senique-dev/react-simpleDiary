...
const DiaryList = ({ data }) => {
  ...
  useEffect(() => {
    // 2) useEffect호출 - 두 번째 인수인 의존성 배열에 data, sortType을 저장. 일기 데이터나 정렬 기준이 바뀌면 첫 번째 인수로 전달한 콜백함수를 다시 실행한다.
    const compare = (a, b) => {
      // 3) 객체 형태의 배열인 data를 최신순 또는 등록순으로 정렬하기 위해 별도의 비교 함수 만들기. 만약 sortType이 latest라면 최신순으로 정렬해야 하므로 일기 객체의 date를 내림차순으로 정렬. date 값이 문자열이므로 Number메소드를 이용해 명시적으로 형변환 후 정렬.
      if (sortType === "latest") {
